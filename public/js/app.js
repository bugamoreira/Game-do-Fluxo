...
// ── Regular chip (PS / RPA / ready lists) ────────────────────
function Chip({ p, sel, onClick }) {
  ...
  return (
    <div className={`chip${sel ? ' selected' : ''}`}
      ...
      title={`${p.name} (${s.l})${p.bMin > 0 ? ` | Boarding: ${Math.floor(p.bMin / 60)}h` : ''}`}
      style={{
  ...
  return (
    <div
      ...
      style={{
        ...
        background: selected ? 'rgba(0,212,255,.18)' : ready ? `${s.c}28` : p.det ? 'rgba(239,68,68,.12)' : 'rgba(255,255,255,.03)',
        border: selected ? '2px solid #00d4ff' : ready ? `1px solid ${s.c}88`
          : p.blocked ? '1px solid #eab30866' : p.social ? '1px solid #a855f766'
          : p.det ? '1px solid #ef444466' : `1px solid ${s.c}22`,
        transition: 'all .15s', position: 'relative',
      }}>
      {ready && <div style={{ position:'absolute', top:2, right:2, width:5, height:5, borderRadius:'50%', background:s.c }}/>}
      {p.det && <div style={{ position:'absolute', top:1, left:1, fontSize:7, color:'#ef4444', lineHeight:1 }}>!</div>}
    </div>
  );
}
...
function SxPan({ surgeries, sm }) {
  return (
    <div style={{ background:'#0f172a', border:'1px solid #1e293b', borderRadius:10, padding:8, flex:1, overflow:'hidden', display:'flex', flexDirection:'column' }}>
      <div style={{ fontSize:10, fontWeight:700, color:'#64748b', marginBottom:5, letterSpacing:'.08em' }}>CC — 4 SALAS</div>
      <div style={{ flex:1, overflowY:'auto' }}>
        {surgeries.map(s => {
          ...
          return (
            <div key={s.id} style={{
              fontSize:10, padding:'4px 6px', borderRadius:5, marginBottom:2,
    ...
    <div style={{ display:'flex', alignItems:'center', justifyContent:'center', minHeight:'100vh', background:'#060a13', padding:20 }}>
      <div style={{ textAlign:'center', maxWidth:440, width:'100%' }}>
        ...
        <div style={{ background:'#0f172a', borderRadius:14, padding:28, border:'1px solid #1e293b', marginBottom:14 }}>
          <div style={{ fontSize:12, fontWeight:700, color:'#64748b', marginBottom:18, letterSpacing:'.08em', textTransform:'uppercase' }}>
            Entrar na Sala Multiplayer
          </div>
          <div style={{ display:'flex', flexDirection:'column', gap:10, marginBottom:16 }}>
            <input
              placeholder="Nome do time (ex: Grupo Alpha)"
              value={tName}
            ...
            <input
              ...
              onKeyDown={handleKey}
              maxLength={8}
              style={{ textTransform:'uppercase', letterSpacing:'.12em', fontWeight:700 }}
            />
          </div>
      ...
      <div style={{ textAlign:'center', maxWidth:620 }}>
        ...
        <div style={{ textAlign:'left', background:'#0f172a', borderRadius:12, padding:20, border:'1px solid #1e293b', marginBottom:22 }}>
          <p style={{ color:'#94a3b8', lineHeight:1.7, marginBottom:10 }}>
            Voce e o <strong style={{ color:'#00d4ff' }}>coordenador de fluxo</strong> de um hospital com 100 leitos. Clique num paciente [OK] para selecionar, depois clique no setor destino para mover.
          </p>
          <p style={{ color:'#94a3b8', lineHeight:1.7 }}>
            O gargalo esta na <strong style={{ color:'#FF3B3B' }}>saida</strong>, nao na entrada. Acima de 85% de ocupacao: colapso exponencial.
          </p>
        </div>
        <div style={{ display:'flex', gap:18, justifyContent:'center', flexWrap:'wrap' }}>
          ...
          <div style={{ flex:1, minWidth:220 }}>
            ...
            <div style={{ fontSize:11, color:'#64748b', lineHeight:1.5 }}>Rodada 2 — ferramentas Lean ativas. Mesma demanda.</div>
          </div>
        </div>
      </div>
    </div>
...
function GameOverModal({ isR2, score, st, pts, onRestart, onMenu }) {
  ...
  const msg = isR2
    ? (st.deaths === 0 && st.cxCan === 0 ? 'As ferramentas Lean transformaram o fluxo. Mesma demanda, resultado oposto. O problema nunca foi falta de leito — era falta de gestao.'
    ...
    : (st.deaths > 0 ? 'O congelamento da saida causou obitos evitaveis. O gargalo nao esta na porta de entrada — esta na porta de saida.'
    ...
    <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,.92)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:100 }}>
      <div style={{ background:'#111827', border:'1px solid #1e293b', borderRadius:16, padding:32, maxWidth:560, width:'92%', textAlign:'center', boxShadow:'0 20px 60px rgba(0,0,0,.6)' }}>
        ...
        <div style={{ display:'flex', gap:10, justifyContent:'center', flexWrap:'wrap' }}>
          ...
          <button onClick={onMenu} className="btn" style={{ background:'#374151', padding:'10px 22px', fontSize:13, fontWeight:700 }}>Menu</button>
        </div>
      </div>
    </div>
  );
...
function Game() {
  ...
  const tick = useCallback(() => {
    ...
    setPts(prev => {
      ...
      // Random events
      if (ch >= 9  && ch < 16 && !E.pcr    && Math.random() < .005) { E.pcr    = true; E.pcrEnd  = cm + 60;  addL('PARADA CARDIACA no PS! 1 maca bloqueada por 1h.', 'danger'); }
      if (E.pcr    && cm >= E.pcrEnd)                                { E.pcr    = false; addL('Parada resolvida. Maca liberada.', 'success'); }
      if (ch >= 8  && ch < 14 && !E.tomo   && Math.random() < .004) { E.tomo   = true; E.tomoEnd = cm + 120; addL('TOMOGRAFO QUEBROU! Decisao +120min para todos no PS.', 'danger'); }
      if (E.tomo   && cm >= E.tomoEnd)                               { E.tomo   = false; addL('Tomografo reparado.', 'success'); }
      if (ch >= 12 && ch < 15 && !E.surto  && Math.random() < .005) {
        E.surto = true;
        for (let i = 0; i < 3; i++) { const d = i < 2 ? rollDest() : { dest:'uti', sev:'red', ps:rnd(90,150) }; const np = mkPt('porta', d.dest, d.sev, false, d.ps); np.arrMin = cm; P.push(np); }
        addL('SURTO! 3 pacientes simultaneos!', 'danger');
      }
      if (ch >= 10 && ch < 17 && !E.social && Math.random() < .004) {
        E.social = true;
        const ef = P.find(p => p.sector === 'enf' && p.dischReady && p.prep <= 0 && !p.social && !p.blocked);
        if (ef) { ef.social = true; ef.socialDelay = isR2local ? 60 : rnd(180, 240); S.socB++; addL(`PACIENTE SOCIAL: ${ef.name} — leito bloqueado ~${Math.round(ef.socialDelay/60)}h.${isR2local ? ' Alta segura acionada.' : ''}`, isR2local ? 'info' : 'warning'); }
      }
      ...
      if (isR2local && cm % 30 === 0) {
        P.filter(p => p.sector === 'ps' && p.ready && p.dest === 'alta_ps').forEach(p => { p.sector = 'alta'; S.disc++; addL(`Fluxista: ${p.name} alta automatica.`, 'success'); });
      }
      ...
      P.forEach(p => {
        if (p.sector === 'ps' && p.ready && (p.dest === 'enf' || p.dest === 'uti')) {
          ...
          p.bMin = cm - p.bStart;
          S.boardHrs += (1 / 60);
          if (p.bMin >= 240 && !p.det) { p.det = true; S.dets++; addL(`DETERIORACAO: ${p.name} — ${Math.floor(p.bMin/60)}h boarding!`, 'warning'); }
          if (p.bMin >= 480 && !p.dead) { p.dead = true; S.deaths++; addL(`OBITO EVITAVEL: ${p.name} — ${Math.floor(p.bMin/60)}h boarding.`, 'danger'); }
        }
      });

      // Off-service deterioration
      P.forEach(p => { if (p.offSvc && p.sector === 'enf' && !p.dead && !p.det && Math.random() < (1/(60*3.33))) { p.det = true; S.dets++; addL(`${p.name} deteriorou OFF-SERVICE!`, 'warning'); } });
      ...
      if (P.filter(p => p.sector === 'corredor').length > 5) {
        const lw = P.find(p => p.sector === 'corredor' && p.dest === 'alta_ps' && !p.dead);
        if (lw && Math.random() < .2) { lw.sector = 'alta'; S.lwbs++; addL(`${lw.name} SAIU SEM ATENDIMENTO!`, 'danger'); }
      }
      ...
      if (ch >= enfRoundH && !R.e1) { R.e1 = true; const c = P.filter(p => p.sector === 'enf' && !p.dischReady && !p.blocked && !p.social); const n = Math.min(rnd(6,8), c.length); for (let i = 0; i < n; i++) { c[i].dischReady = true; c[i].prep = isR2local ? 0 : (Math.random() < .2 ? 120 : 60); } addL(`ROUND ${enfRoundH}h: ${n} altas prescritas.${isR2local ? ' Discharge Lounge: leitos liberam IMEDIATO.' : ' Preparo ~1h.'}`, 'success'); }
      if (ch >= 14 && !R.e2) { R.e2 = true; const c = P.filter(p => p.sector === 'enf' && !p.dischReady && !p.blocked && !p.social); const n = Math.min(rnd(3,4), c.length); for (let i = 0; i < n; i++) { c[i].dischReady = true; c[i].prep = isR2local ? 0 : 60; } addL(`ROUND 14h: ${n} altas.${isR2local ? ' Liberacao imediata.' : ''}`, 'success'); }
        ...
        sx2.forEach(s => {
          ...
          if (cm >= enM) {
            ...
            if (rC >= CAP.rpa) {
              ...
              if (disch) { disch.dischReady = false; disch.blocked = true; addL(`${s.label} CANCELADA! ${disch.name} perde alta. CASCATA.`, 'danger'); setCascade(`${s.label} cancelada > ${disch.name} perde alta > leito nao gira`); setTimeout(() => setCascade(null), 6000); }
              else { addL(`${s.label} CANCELADA — RPA lotada!`, 'danger'); setCascade('Cirurgia cancelada > menos giro > mais boarding'); setTimeout(() => setCascade(null), 5000); }
            } else {
              s.st = 'done';
  ...
  const doMove = sid => {
    ...
    setPts(prev => prev.map(p => {
      ...
      if (sid === 'alta') setSt(s => ({ ...s, disc: s.disc + 1 }));
      else if (isOff)     setSt(s => ({ ...s, offS: s.offS + 1 }));
      if (sid === 'alta') addL(`${p.name} alta.`, 'success');
      else if (isOff)     addL(`OFF-SERVICE: ${p.name} UTI>ENF. Risco!`, 'warning');
  ...
  // ── Phase routing ──────────────────────────────────────────
  if (ph === 'lobby')   return <LobbyScreen onJoin={joinRoom} onSolo={() => setPh('menu')}/>;
  ...
  return (
    <div style={{ height:'100vh', display:'flex', flexDirection:'column', overflow:'hidden', background:'#060a13' }}>
      ...
      {/* ── Header ── */}
      <div style={{ background:'#0a0f1a', borderBottom:'1px solid #1e293b', padding:'6px 14px', flexShrink:0 }}>
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', gap:8 }}>
          <div style={{ display:'flex', alignItems:'center', gap:10 }}>
            ...
            <span style={{ fontSize:10, fontWeight:700, padding:'2px 8px', borderRadius:4,
              background: isR2 ? 'rgba(0,212,255,.15)' : 'rgba(255,59,59,.15)',
              color: isR2 ? '#00d4ff' : '#FF3B3B' }}>R{rnd2}</span>
      ...
      {/* ── Main area (4 columns, fills remaining height) ── */}
      <div style={{ flex:1, display:'flex', gap:6, padding:'6px 8px', overflow:'hidden', minHeight:0 }}>
        ...
        <div style={{ width:330, display:'flex', flexDirection:'column', gap:6, minHeight:0 }}>
          ...
          <div className={`sector${tgts.includes('ps') ? ' valid-target' : ''}${fl === 'ps' ? ' flash' : ''}`}
            onClick={() => doMove('ps')}
            style={{ flex:1, background:'#0f172a', border:'1px solid #1e293b', cursor: tgts.includes('ps') ? 'pointer' : 'default', display:'flex', flexDirection:'column', minHeight:0 }}>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:6, flexShrink:0 }}>
              ...
              <span style={{ fontSize:12, fontFamily:'monospace', fontWeight:700,
                color: pctOf(psOcc,CAP.ps) >= 100 ? '#ef4444' : pctOf(psOcc,CAP.ps) >= 85 ? '#eab308' : '#64748b' }}>
                {psOcc}/{CAP.ps} ({pctOf(psOcc,CAP.ps)}%){psMult(psOcc) > 1 ? ` [x${psMult(psOcc)}]` : ''}
              </span>
            </div>
            <div style={{ flex:1, display:'flex', flexDirection:'column', gap:5, minHeight:0, overflow:'hidden' }}>
              ...
              <div className="subarea" style={{ background:'rgba(234,179,8,.04)', border:'1px solid #eab30822' }}>
                <div style={{ fontSize:9, color:'#eab308', marginBottom:3 }}>Boarding ({psBoard.length})</div>
              ...
              {psObs.length > 0 && (
                <div className="subarea" style={{ background:'rgba(168,85,247,.04)', border:'1px solid #a855f722' }}>
          ...
          <div className="sector" style={{
            ...
            border: `1px solid ${byS('corredor').length > 5 ? '#ef4444' : byS('corredor').length > 0 ? '#eab308' : '#1e293b'}`,
            flexShrink:0,
          }}>
            <div style={{ fontSize:10, fontWeight:700, color: byS('corredor').length > 0 ? '#ef4444' : '#64748b', marginBottom:3 }}>
              CORREDOR{byS('corredor').length > 0 ? ` (${byS('corredor').length}!)` : ''}
            </div>
            <div style={{ display:'flex', flexWrap:'wrap', gap:3, minHeight:16 }}>
        ...
        <div style={{ flex:1, display:'flex', flexDirection:'column', gap:6, minHeight:0 }}>
          ...
          <div className={`sector${tgts.includes('enf') ? ' valid-target' : ''}${fl === 'enf' ? ' flash' : ''}`}
            ...
            {enfReady.length > 0 && (
              <div style={{ fontSize:9, color:'#22c55e', marginTop:4, flexShrink:0 }}>
        ...
        <div style={{ width:205, display:'flex', flexDirection:'column', gap:6, minHeight:0 }}>
          ...
          <div className={`sector${tgts.includes('alta') ? ' valid-target' : ''}`}
            ...
            <div style={{ fontSize:36, fontWeight:900, color:'#22c55e', fontFamily:'monospace', lineHeight:1 }}>{st.disc}</div>
          ...
          <div style={{ flex:1, background:'#0a0f1a', border:'1px solid #1e293b', borderRadius:10, padding:8, overflowY:'auto', minHeight:0 }}>
            ...
            {log.map((l, i) => (
              <div key={i} className="log-entry"
                style={{ background:logBg[l.type], borderLeft:`3px solid ${logBrd[l.type]}`, color:logC[l.type], animation: i === 0 ? 'fadeIn .3s' : 'none' }}>
      ...
      {sel && run && (
        <div style={{ position:'fixed', bottom:12, left:'50%', transform:'translateX(-50%)', background:'#1e293b', border:'2px solid #00d4ff', borderRadius:12, padding:'8px 16px', zIndex:50, boxShadow:'0 8px 40px rgba(0,0,0,.6)', display:'flex', alignItems:'center', gap:12, animation:'fadeIn .2s', maxWidth:'92vw' }}>
          ...
          <div style={{ minWidth:0 }}>
            ...
            {sel.bMin > 0 && (
              <div style={{ fontSize:10, fontWeight:700, color: sel.bMin >= 480 ? '#ef4444' : sel.bMin >= 240 ? '#f97316' : '#eab308' }}>
          ...
          <div style={{ display:'flex', gap:4, marginLeft:6, flexShrink:0, flexWrap:'wrap' }}>
            ...
            {isR2 && sel.sector === 'ps' && sel.ready && sel.dest !== 'alta_ps' && nirUses < 3 && nirCd <= 0 && (
              <button onClick={doNIR} className="btn" style={{ background:'#7c3aed', fontSize:10 }}>NIR (transferir)</button>
            )}
      ...
      {/* ── Game over modal ── */}
      {ph === 'over' && (
        <GameOverModal isR2={isR2} score={score} st={st} pts={pts}
          onRestart={startR} onMenu={() => setPh('menu')}/>
      )}
    </div>
  );
}
